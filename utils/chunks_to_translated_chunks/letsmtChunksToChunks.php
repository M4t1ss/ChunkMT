<?php
// This is the part of the Syntactic Multi-System Hybrid Translator workflow that translates a chunked input to a chunked translated output with LetsMT
// It requires two parameters - chunked input file name & output file name
// It is run with the following command:
// php letsmtChunksToChunks.php <input chunked> <output chunked>
// For example:
// php letsmtChunksToChunks.php chunked.txt chunked.translated.txt
...
while (($input = fgets($inputFile)) !== false) {
	if($input == "\n"){
		...
		$translatedSentence = translateWithLetsMT($input);
